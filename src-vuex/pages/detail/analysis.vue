<template>
  <div class="sales-board">
      ...
      <my-dialog  :is-show="isShowErrDialog"  @on-close="closeDialog('isShowErrDialog')">
        支付失败！
      </my-dialog>
...
export default {
  ...
  methods:{
    ...
    getPrice(){
      ...
      this.$http.post('/api/getPrice',passParams).then((res)=>{
       ...
       // let data=JSON.parse(res.data)//强制转换成json,后端返回的是字符串的话是需要强制转换的
  ...
  mounted(){//初始化
    ...
    this.versions = [this.versionList[0]]//不能改变数据类型，是数组
